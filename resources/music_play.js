...
const { canModifyQueue, STAY_TIME, LOCALE, SPOTIFY_API_ID } = require("./musicutil");
...
module.exports = {
    async play(song, message) {
        const { SOUNDCLOUD_CLIENT_ID } = require("../settings.json");
        ...
        if (!song) {
            setTimeout(function () {
                if (queue.connection.dispatcher && message.guid.me.voice.channel) return;
                queue.channel.leave();
        ...
        spotifyUri_pattern = /(spotify:track:)([a-zA-Z0-9]+)(.*)$/gm;
        ...
        try {
            if (song.url.includes("youtube.com")) {
                ...
                try {
                    sream = await scdl.downloadFormat(song.url, scdl.FORMATS.OPUS, SOUNDCLOUD_CLIENT_ID);
                } catch (error) {
                    stream = await scdl.downloadFormat(song.url, scdl.FORMATS.MP3, SOUNDCLOUD_CLIENT_ID);
                    streamType = "unknown";
                }
            } else if (song.url.includes("spotify.com")) {
                stream = await spdl(song.url, {opusEncoded: true});
                streamType = "opus"
            } else if (song.url.includes("discordapp.com")) {
                stream = song.url;
                streamType = "unknown";
            }
        } catch (error) {
        ...
        collector.on("collect", (reaction, user) => {
            ...
            switch (reaction.emoji.name) {
                ...
                case "🔉":
                    ...
                    queue.textChannel.messages.fetch({ limit: 1 }).then(messages => {
                        let lastMessage = messages.first();
                        if (lastMessage.author.bot && lastMessage.content.includes("volume")) {
                            lastMessage.edit(i18n.__mf("play.decreasedVolume", { author: user, volume: queue.volume })).catch(console.error);
                        }
                        else {
                ...
                case "🔊":
                    ...
                    queue.textChannel.messages.fetch({ limit: 1 }).then(messages => {
                        let lastMessage = messages.first();
                        if (lastMessage.author.bot && lastMessage.content.includes("volume")) {
                            lastMessage.edit(i18n.__mf("play.increasedVolume", { author: user, volume: queue.volume })).catch(console.error);
                        }
                        else {
