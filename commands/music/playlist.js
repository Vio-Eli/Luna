...
const {
    ...
    DEFAULT_VOLUME,
    LOCALE
} = require("../../resources/musicutil");

const youtube = new YouTubeAPI(YOUTUBE_API_KEY);
const spotify = new SpotifyAPI.Client()
spotify.login(SPOTIFY_API_ID, SPOTIFY_API_SECRET)
...
module.exports = {
    ...
    async execute(Glossary, client, message, args) {
        ...
        const spotifyPattern = /^(https:\/\/open.spotify.com\/.*playlist\/|spotify:user:spotify:playlist:)([a-zA-Z0-9]+)(.*)$/gm;
        ...
        if (urlValid) {
            ...
            playlist_pattern = spotifyPattern.exec(url)
            playlist = await spotify.playlists.getTracks(playlist_pattern[2]);
            videos = playlist.items.map((track) => ({
